...
def ShoppingCart():
    ...
    def saveCart():
        import csv
        with open("shoppingCart.csv", mode="w+", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=['Items'])
            writer.writeheader()
            writer = csv.writer(f)
            for i in range(len(cart)):
                writer.writerow([cart[i]])
        print("Cart has been saved\n\n\n")

        with open("shoppingCart.csv", mode="r", newline="") as r:
            reader = csv.reader(r)
            print("Your cart is: \n")
            for row in reader:
                print(row)
    ...
    def main():
        ...
        while not done:
            ans = input("quit/add/remove/show/saveCart/clear: ").lower()
            if ans == "quit":
